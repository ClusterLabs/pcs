...
class ResourceRestart(TestCase):
    def setUp(self):
        self.lib = mock.Mock(spec_set=["resource"])
        self.resource = mock.Mock(spec_set=["restart", "is_any_stonith"])
        self.resource.is_any_stonith.return_value = False
        self.lib.resource = self.resource
    ...
    def test_stonith(self, mock_deprecation, mock_print):
        ...
        resource.resource_restart_cmd(
            self.lib, ["stonith"], dict_to_modifiers({})
        )
        self.resource.restart.assert_called_once_with("stonith", None, None)
        mock_print.assert_called_once_with("stonith successfully restarted")
        mock_deprecation.assert_called_once_with(
