...
class TestSetProperty(TestCase):
    def setUp(self):
        ...
        self.cluster_property = mock.Mock(spec_set=["set_property"])
    ...
    def test_no_args(self):
        ...
        self.cluster_property.set_property.assert_not_called()
    ...
    def test_multiple_args(self):
        ...
        self.cluster_property.set_property.assert_called_once_with(
            {"a": "1", "b": "2", "c": ""}, set()
        )
...
class TestUnsetProperty(TestCase):
    def setUp(self):
        self.lib = mock.Mock(spec_set=["cluster_property"])
        self.cluster_property = mock.Mock(spec_set=["unset_property"])
        self.lib.cluster_property = self.cluster_property
    ...
    def test_no_args(self):
        with self.assertRaises(CmdLineInputError) as cm:
            self._call_cmd([])
        self.assertIsNone(cm.exception.message)
        self.cluster_property.unset_property.assert_not_called()

    def test_args(self):
        self._call_cmd(["a=1", "=b", ""])
        self.cluster_property.unset_property.assert_called_once_with(
            ["a=1", "=b", ""], set()
        )
