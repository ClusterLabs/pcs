...
class GetNodeSbdConfigTextTest(TestCase):
    ...
    def _assert_failure(self, reason):
        self.env_assist.assert_raise_library_error(
            lambda: cmd_sbd.get_node_sbd_config_text(self.env_assist.get_env()),
        )
    ...
    def test_open_failure(self):
        reason = "open error"
        self.config.fs.open(settings.sbd_config, side_effect=OSError(reason))
        self._assert_failure(reason)

    def test_flock_failure(self):
        reason = "flock error"
        mock_file = fixture.get_mock_file()
        self.config.fs.open(settings.sbd_config, mock_file)
    ...
    def test_read_failure(self):
        reason = "read error"
        mock_file = fixture.get_mock_file()
        mock_file.read.side_effect = OSError(reason)
        self.config.fs.open(settings.sbd_config, mock_file)
        self.config.fcntl.flock(mock_file, fcntl.LOCK_SH)
        self._assert_failure(reason)
...
class SetNodeSbdConfigTextTest(TestCase):
    ...
    def test_success(self):
        ...
        mock_file = fixture.get_mock_file()
        self.config.fs.open(settings.sbd_config, mock_file, mode="w")
        self.config.fcntl.flock(mock_file, fcntl.LOCK_EX)
        cmd_sbd.set_node_sbd_config_text(self.env_assist.get_env(), config_data)
        mock_file.write.assert_called_once_with(config_data)
    ...
    def test_open_failure(self):
        reason = "reason"
        self.config.fs.open(
            settings.sbd_config, side_effect=OSError(reason), mode="w"
        )
        self._assert_failure(reason)
    ...
    def test_flock_failure(self):
        ...
        self.config.fs.open(settings.sbd_config, mock_file, mode="w")
        self.config.fcntl.flock(
            mock_file, fcntl.LOCK_EX, side_effect=OSError(reason)
        )
        self._assert_failure(reason)
    ...
    def test_write_failure(self):
        reason = "write error"
        mock_file = fixture.get_mock_file()
        mock_file.write.side_effect = OSError(reason)
        self.config.fs.open(settings.sbd_config, mock_file, mode="w")
        self.config.fcntl.flock(mock_file, fcntl.LOCK_EX)
        self._assert_failure(reason)
