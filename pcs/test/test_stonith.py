...
""")
    ...
    def test_stonith_create_compute_aliases(self):
        self.assert_pcs_success(
            "stonith create test1 fence_compute auth_url=test1 project_domain=val1 user_domain=val2 compute_domain=val3",
            outdent(
                """\
                Warning: Using 'project-domain' instead of 'project_domain'
                Warning: Using 'compute-domain' instead of 'compute_domain'
                Warning: Using 'user-domain' instead of 'user_domain'
                """
            )
        )
        self.assert_pcs_success(
            ...
            outdent(
                """\
                 Resource: test1 (class=stonith type=fence_compute)
                  Attributes: auth_url=test1 compute-domain=val3 project-domain=val1 user-domain=val2
                  Operations: monitor interval=60s (test1-monitor-interval-60s)
                """
            )
        )
        self.assert_pcs_success(
            "stonith create test2 fence_compute auth_url=test2 project-domain=val1 user-domain=val2 compute-domain=val3",
        )
        self.assert_pcs_success(
            "stonith show --full",
            outdent(
                """\
                 Resource: test1 (class=stonith type=fence_compute)
                  Attributes: auth_url=test1 compute-domain=val3 project-domain=val1 user-domain=val2
                  Operations: monitor interval=60s (test1-monitor-interval-60s)
                 Resource: test2 (class=stonith type=fence_compute)
                  Attributes: auth_url=test2 compute-domain=val3 project-domain=val1 user-domain=val2
                  Operations: monitor interval=60s (test2-monitor-interval-60s)
                """
            )
        )
        self.assert_pcs_fail(
            "stonith create test3 fence_compute auth_url=test1 project_domain=val1 project-domain=val1 user_domain=val2 compute_domain=val3",
            outdent(
                """\
                Error: Only one of options 'project-domain' and 'project_domain' can be used
                Warning: Using 'compute-domain' instead of 'compute_domain'
                Warning: Using 'user-domain' instead of 'user_domain'
                """
            )
    ...
    def test_stonith_update_compute_aliases(self):
        shutil.copy(rc("cib-fence_compute.xml"), temp_cib)
        self.assert_pcs_success(
            "stonith show --full",
            outdent(
                """\
                 Resource: test (class=stonith type=fence_compute)
                  Attributes: auth_url=test1 compute_domain=val3 project_domain=val1 user_domain=val2
                  Operations: monitor interval=60s (test-monitor-interval-60s)
                """
            )
        )
        self.assert_pcs_fail(
            "stonith update test auth_url=val0 project_domain=val2 project-domain=val3",
            "Error: Only one of options 'project-domain' and 'project_domain' can be used\n"
        )
        self.assert_pcs_success(
            "stonith update test auth_url=val0 project-domain=val3",
        )
        self.assert_pcs_success(
            "stonith show --full",
            outdent(
                """\
                 Resource: test (class=stonith type=fence_compute)
                  Attributes: auth_url=val0 compute_domain=val3 user_domain=val2 project-domain=val3
                  Operations: monitor interval=60s (test-monitor-interval-60s)
                """
            )
        )
        self.assert_pcs_success(
            "stonith update test auth_url=val1 project_domain=val2 user_domain=val3 compute_domain=val4",
            outdent(
                """\
                Warning: Using 'project-domain' instead of 'project_domain'
                Warning: Using 'compute-domain' instead of 'compute_domain'
        ...
        self.assert_pcs_success(
            ...
            outdent(
                """\
                 Resource: test (class=stonith type=fence_compute)
                  Attributes: auth_url=val1 project-domain=val2 user-domain=val3 compute-domain=val4
                  Operations: monitor interval=60s (test-monitor-interval-60s)
                """
            )
        )
