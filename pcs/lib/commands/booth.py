...
def ticket_cleanup(env: LibraryEnvironment, ticket_name: str) -> None:
    ...
    report_processor.report_list(_ticket_standby(cmd_runner, ticket_name))
    if report_processor.has_errors:
        raise LibraryError()

    report_processor.report(
        reports.ReportItem.info(
            reports.messages.BoothTicketCleanup(ticket_name)
        )
    )
    stdout, stderr, retval = live_ticket_cleanup(cmd_runner, ticket_name)
    if retval != 0:
        report_processor.report(
