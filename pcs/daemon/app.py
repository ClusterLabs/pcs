...
class PcsdStaticFileHandler(StaticFileHandler):
...
def create_static_handler(url_prefix, public_subdir):
    return (
        r"{}(.*)".format(url_prefix),
        PcsdStaticFileHandler,
        dict(
...
def make_app():
    return Application(
        [
            ...
            create_static_handler("/css/", "css"),
            create_static_handler("/js/", "js"),
            create_static_handler("/images/", "images"),
            ...
            # Always accessible gui/ajax urls (i.e. all the login/logouts)
            (r"/login", Login),
...
def main():
    ...
    http_server = HTTPServer(
        make_app(),
        ssl_options={
            "certfile": "/var/lib/pcsd/pcsd.crt",
            "keyfile": "/var/lib/pcsd/pcsd.key",
        }
    )
    http_server.listen(3224)
